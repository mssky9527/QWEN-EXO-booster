...
_is_npu = is_npu()
_DFLASH_DISABLE_TORCH_COMPILE = (
    _is_npu or os.environ.get("SGLANG_DFLASH_DISABLE_TORCH_COMPILE", "0") == "1"
)
_DFLASH_COMPILE_BACKEND = (
    "eager" if _DFLASH_DISABLE_TORCH_COMPILE else get_compiler_backend()
)
try:
    from sglang.srt.speculative.dflash_utils import table_qk_norm_rope_
except ImportError:
    table_qk_norm_rope_ = None
if _is_npu:
...
class DFlashAttention(nn.Module):
    def __init__(self, config, layer_id: int, quant_config=None) -> None:
        ...
        self.use_table_qk_norm_rope = (
            table_qk_norm_rope_ is not None
            and not _is_npu
...
@torch.compile(
    dynamic=True,
    backend=_DFLASH_COMPILE_BACKEND,
    disable=_DFLASH_DISABLE_TORCH_COMPILE,
)
