...
SERVICE_SETTINGS = (
    _setting(
        ...
        "capacity",
        "上下文长度",
        "作用：限制单个请求可使用的最大上下文长度。用途：防止超长请求挤占显存；调大可支持更长对话。推荐值：131072（双卡 128K 配置）。",
        "integer",
        131072,
        minimum=4096,
        maximum=262144,
    ...
    _setting(
        ...
        "memory",
        "最大候选数",
        "作用：限制初始检索及非 QK 补充候选数量；通过预过滤的 Attention-Q×K 候选全部进入语义准入，首轮全拒绝时再进行一次有界扩张。弱分数候选仍按 fail-closed 规则直接放弃。用途：控制召回范围与裁判耗时。推荐值：8。",
        "integer",
        8,
        minimum=1,
        maximum=64,
    ...
    _setting(
        ...
        "tensor_bank",
        "文档长度上限",
        "作用：单个文档允许编译入库的最大 token 数；不得超过当前上下文长度减 2048。当前 128K 上下文仍使用保守值 100352。",
        "integer",
        100352,
        minimum=64,
        maximum=131072,
    ...
    _setting(
        ...
        "score_bias",
        "轨迹相关性下限",
        "作用：轨迹块相关性余弦分数下限。默认值：0.01；设为 0 可关闭正相关门槛。",
        "number",
        0.01,
        minimum=-1,
        maximum=1,
    ...
    _setting(
        ...
        "score_bias",
        "系统/工具锚点偏置",
        "作用：对原始 system instructions 与工具 schema 的少量 token span 提供有界保护，默认值：0.01；设为 0 才关闭。",
        "number",
        0.01,
        minimum=0,
        maximum=0.05,
    ...
    _setting(
        ...
        "reflection_memory",
        "历史预算",
        "作用：反思记忆可读取的轨迹来源总预算。当前服务上下文为 128K，默认仍保守使用 92160，以预留系统提示和输出空间；超长轨迹按事件去重并保留最近证据，同时记录省略审计。",
        "integer",
        92160,
        minimum=1024,
        maximum=96256,
